...
enum FlightModes { BOARD,
                   MANUAL_ORIENTATION_WORLD_INCREMENTAL, MANUAL_ORIENTATION_WORLD_ABSOLUTE,
                   MANUAL_ORIENTATION_BODY_INCREMENTAL, MANUAL_ORIENTATION_BODY_ABSOLUTE,
                   MANUAL_POSITION_INCREMENTAL, MANUAL_POSITION_ABSOLUTE,

                   OPEN_LOOP_FORCE,
                   OPEN_LOOP_TORQUE_WORLD, OPEN_LOOP_TORQUE_BODY};  //1099 is BOARD, 1500 is MANUAL and 1901 is OPEN_LOOP
...
void manualControl_OrientationBodyIncremental(){  //it is multiplied by a constant (90/400) because I want a full stick up(or down) to mean 90 degrees in angle change. Also, the range of stick movement in one side is 400 (1100-1500). Hence each unit of stick movement change should give stick_input*(90/400) in angle change. Hence Result!!!
...
void manualControl_OrientationWorldAbsolute(Eigen::Quaterniond current_orientation_manual_absolute){ //Explanation: the reason to include desired orientation in the manual absolute in the world frame is because in the equation rc_Rd = Ri*rc_Rd, rd_Rd is constantly being updated thus defeating the purpose of using a central orientation around whichthe absolute values of rotation are applied
    ...
    if(1){  ///rc_flag more like rc_lock for orientation----- so that the rc_Rd is given the value of current orientation only when its flicked to manual mode-- after that it takes the value of the ri*rd not current orientation
        tf::Quaternion q_temp_cur;
        q_temp_cur.setX(current_orientation_manual_absolute.x());
        q_temp_cur.setY(current_orientation_manual_absolute.y());
        q_temp_cur.setZ(current_orientation_manual_absolute.z());
...
int main(int argc, char** argv) {
    ...
    ros::Subscriber imu_values = nh.subscribe("/mavros/imu/data", 1000, imuCallback);

   ros::Publisher pub_force =  nh.advertise<geometry_msgs::Vector3Stamped>
            ("/open_loop/force", 1000);

   ros::Publisher pub_torque = nh.advertise<geometry_msgs::Vector3Stamped>
            ("/open_loop/torque", 1000);
    ...
    nh.param<double>("incremental_mode", incremental);
    nh.param<vector<double>>("force_ol", force_ol);
    nh.param<vector<double>>("moment_ol", torque_ol);
    ...
    while(ros::ok()) {
        ...
        nh.getParam("incremental_mode", incremental);
        nh.getParam("force_ol", force_ol);
	      nh.getParam("moment_ol", torque_ol);
        ...
        *********************************************************************************************************/
        ...
        nh.setParam("desired_orientation_rpy", orientation_desired_rpy);
            ...
            nh.setParam("desired_position", position_desired);
        }
        ...
        //////////////////////////////////////                 ////////////////////////////////////////////////////////
            ...
            if(flight_mode == FlightModes::OPEN_LOOP_TORQUE_WORLD){
                ...
                /// setting value of torque from transmitter depending on the difference in the sticks
                OpenLoopTorqueWorld(moment);

            }

            if(flight_mode == FlightModes::OPEN_LOOP_TORQUE_BODY){
        ...
        geometry_msgs::Vector3Stamped opl_force, opl_torque;
        //opl_force.header.stamp = ros::Time::now();
        opl_force.vector.x = force[0];
        opl_force.vector.y = force[1];
        opl_force.vector.z = force[2];

        //opl_torque.header.stamp = ros::Time::now();
