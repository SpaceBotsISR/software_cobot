//
//  A_Matrix.cpp
//  RPMConversion
//
//  Created by Filipe Rosa on 29/06/2017.
//  Copyright © 2017 Space CoBot. All rights reserved.
//

#include "AMatrix.h"

/**
 Function which converts robot Force and Torque into the amount of rotation of each motor

 @param force 3x1 Force vector
 @param torque 3x1 Torque vector
 @return U - 6x1 actuation vector where sqrt(|ui|)*sign(ui) corresponds to rev/s (revolutions per second)
 */
Eigen::VectorXf getActuationVector(Eigen::Vector3f force, Eigen::Vector3f torque){
    Eigen::VectorXf FM(6);
    FM << force, torque;
    Eigen::MatrixXf AiMatrix(6,6);
    AiMatrix << -6.776660281601378126e-17, -4.069248629204854639e-01, 2.905744659368497129e-01, -1.897464878848385925e-15, -3.334542096204833328e+00, -1.329837852169367229e+00,
                -3.524072687206405985e-01, 2.034624314602428152e-01, 2.905744659368497129e-01, 2.887798165301998399e+00, -1.667271048102417330e+00, 1.329837852169366563e+00,
                3.524072687206407650e-01, 2.034624314602425932e-01, 2.905744659368496019e-01, 2.887798165302000175e+00, 1.667271048102416220e+00, -1.329837852169366341e+00,
                -1.107312451872372199e-16, -4.069248629204853529e-01, 2.905744659368496574e-01, 4.429249807489488797e-16, 3.334542096204833328e+00, 1.329837852169366563e+00,
                -3.524072687206407095e-01, 2.034624314602426765e-01, 2.905744659368496019e-01, -2.887798165301998843e+00, 1.667271048102417108e+00, -1.329837852169367229e+00,
                3.524072687206407650e-01, 2.034624314602426765e-01, 2.905744659368496019e-01, -2.887798165301999287e+00, -1.667271048102416220e+00, 1.329837852169366785e+00;

    return AiMatrix*FM;
}

Eigen::Matrix3f getRotationMatrix(Eigen::VectorXf q_current){
    Eigen::Matrix3f R;
    R << powf(q_current(0),2)+powf(q_current(1),2)-powf(q_current(2),2)-powf(q_current(3),2), 2*(q_current(1)*q_current(2) - q_current(0)*q_current(3)), 2*(q_current(1)*q_current(3)+ q_current(0)*q_current(2)),
            2*(q_current(1)*q_current(2)+q_current(0)*q_current(3)), powf(q_current(0),2)-powf(q_current(1),2)+powf(q_current(2),2)-powf(q_current(3),2), 2*(q_current(2)*q_current(3) - q_current(0)*q_current(1)),
            2*(q_current(1)*q_current(3)-q_current(0)*q_current(2)), 2*(q_current(2)*q_current(3)+q_current(0)*q_current(1)), powf(q_current(0),2)-powf(q_current(1),2)-powf(q_current(2),2)+powf(q_current(3),2);


    return R;
}