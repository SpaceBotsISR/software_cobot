...
void loop(void)
{
    ...
    if (counter>0 && (now - motor_init >= 2000) /* 100ms : 10hz */) {//WAIT 2 SECONDS from motor initialization
        ...
        double current_time = AP_HAL::millis();
        double t = (current_time-init_time)/1000; //convert to seconds
        double f[] = {0.7500, //f0
                      0.9526, //f1
                      1.2100, //f2
                      1.5369, //f3
                      ...
                      2.4794, //f5
                      3.1492, //f5
                      4.0000};//f7  sinusoid frequency
        double pi = 3.14159265359;
        double max_torque = 0.2;

        double torque = sin(2*pi*f[7]*t)*max_torque;//SINUSOID TORQUE
        Eigen::Vector3d M = Eigen::Vector3d(0,0,torque);

        hal.console->printf("time t= %f  f= %f \n", t, f);
