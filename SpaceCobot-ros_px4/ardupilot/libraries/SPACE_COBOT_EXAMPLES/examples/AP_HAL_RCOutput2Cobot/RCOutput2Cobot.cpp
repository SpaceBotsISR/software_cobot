...
void drive_custom(uint16_t hz_speed, uint16_t pwm);
...
void drive_custom(uint16_t hz_speed, uint16_t my_pwm){
    hal.rcout->set_freq(0xFF, hz_speed);
    hal.rcout->write(1, my_pwm);
}  
...
int8_t Menu_Commands::menu_esc(uint8_t argc, const Menu::arg *argv) {
    uint16_t my_arg = strtol(argv[1], NULL, 10);
    hal.console->printf("Selected pwm:%d\n",my_arg);
    drive_custom(ESC_HZ, arg);
    return 0;
}
