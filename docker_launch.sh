  ...
  "run")
    ...
    if [ "$(docker ps -q -f name=scobot)" ]; then
      ...
      if [ "$(docker ps -aq -f name=scobot)" ]; then
        ...
        docker run -it --network=host --ipc=host -e DISPLAY=$DISPLAY -v $PWD/ros2_ws:/ros2_ws -e DISPLAY=host.docker.internal:0 -v /tmp/.X11-unix:/tmp/.X11-unix:rw -t -i --device=/dev/ttyACM0 --name scobot scobot_ros2
      fi
    fi
    ;;
  "exec")
