...
pt::ptree
pose_stamped_to_ptree(const geometry_msgs::msg::PoseStamped &pose_stamped) {
    pt::ptree tree;
...
pt::ptree
execution_summary_to_ptree(const nav6d::msg::PathExecutionSummary &msg) {
    pt::ptree tree;
...
geometry_msgs::msg::Pose ptree_to_pose(const pt::ptree &pose_tree) {
    ...
    auto get_coord = [source](const std::string &key,
...
ptree_to_pose_stamped(const pt::ptree &pose_tree) {
...
nav_msgs::msg::Path ptree_to_path(const pt::ptree &path_tree) {
    ...
    path_msg.header.frame_id =
        path_tree.get<std::string>("frame_id", path_msg.header.frame_id);
...
class TeleopNode : public rclcpp::Node {
  public:
    TeleopNode()
        : Node("teleop_node"), context_(1), sensor_pub_(context_, ZMQ_PUB),
          image_pub_(context_, ZMQ_PUB), cmd_sub_(context_, ZMQ_SUB) {
        declare_parameter<std::string>("teleop_bridge_sensor_endpoint",
                                       kDefaultSensorEndpoint);
        declare_parameter<std::string>("teleop_bridge_image_endpoint",
                                       kDefaultImageEndpoint);
        declare_parameter<std::string>("teleop_cmd_endpoint",
                                       kDefaultCommandEndpoint);
        ...
        const auto sensor_endpoint =
            get_parameter("teleop_bridge_sensor_endpoint").as_string();
        const auto image_endpoint =
            get_parameter("teleop_bridge_image_endpoint").as_string();
        const auto cmd_endpoint =
            get_parameter("teleop_cmd_endpoint").as_string();
        ...
        nav_path_pub_ =
            create_publisher<nav_msgs::msg::Path>("/nav6d/planner/path", 10);

        imu_sub_ = create_subscription<sensor_msgs::msg::Imu>(
            "/imu/data", 10, [this](sensor_msgs::msg::Imu::ConstSharedPtr msg) {
                handle_imu(msg);
            });
        ...
        nav_path_sub_ = create_subscription<nav_msgs::msg::Path>(
            ...
            [this](nav_msgs::msg::Path::ConstSharedPtr msg) {
                handle_nav6d_path(msg);
            });

        path_quality_sub_ = create_subscription<nav6d::msg::PathQuality>(
            "/nav6d/path_quality", 10,
            [this](nav6d::msg::PathQuality::ConstSharedPtr msg) {
                handle_path_quality(msg);
            });

        velocity_summary_sub_ =
            create_subscription<nav6d::msg::PathExecutionSummary>(
                ...
                [this](nav6d::msg::PathExecutionSummary::ConstSharedPtr msg) {
                    handle_execution_summary(
                        "/nav6d/velocity_controller/path_execution_summary",
                        msg);
                });
        ...
        force_summary_sub_ =
            create_subscription<nav6d::msg::PathExecutionSummary>(
                ...
                [this](nav6d::msg::PathExecutionSummary::ConstSharedPtr msg) {
                    handle_execution_summary(
                        "/nav6d/force_controller/path_execution_summary", msg);
                });
        ...
        nav_goal_pub_ = create_publisher<geometry_msgs::msg::PoseStamped>(
            "/nav6d/goal", 10);
        ...
        RCLCPP_INFO(
            ...
            "(cmd frame=%s, default input=%s, one-way delay=%dms)",
            sensor_endpoint.c_str(), image_endpoint.c_str(),
            cmd_endpoint.c_str(), command_frame_.c_str(),
            default_input_frame_.c_str(), delay_ms);
    }
    ...
    ~TeleopNode() override {
        try {
            ...
            RCLCPP_WARN(get_logger(), "Failed to close ZMQ sockets cleanly: %s",
                        e.what());
        }
    }

  private:
    void configure_socket(zmq::socket_t &socket, const std::string &endpoint,
                          int hwm) {
    ...
    void configure_cmd_socket(zmq::socket_t &socket,
    ...
    void publish_payload(const std::string &topic, const pt::ptree &payload,
                         bool is_image) {
        pt::ptree envelope;
    ...
    void send_payload_message(const std::string &topic,
                              const std::string &message, bool is_image) {
        try {
            ...
            RCLCPP_WARN(get_logger(),
                        "Failed to publish teleop payload for topic %s: %s",
                        topic.c_str(), e.what());
        }
    }
    ...
    void flush_delayed_payloads() {
        ...
        while (!queued_payloads_.empty() &&
               queued_payloads_.front().ready_at <= now) {
    ...
    void handle_imu(sensor_msgs::msg::Imu::ConstSharedPtr msg) {
        ...
        payload.add_child("orientation_covariance",
                          array_to_ptree(msg->orientation_covariance));
        payload.add_child("angular_velocity",
                          vector3_to_ptree(msg->angular_velocity));
        payload.add_child("angular_velocity_covariance",
                          array_to_ptree(msg->angular_velocity_covariance));
        payload.add_child("linear_acceleration",
                          vector3_to_ptree(msg->linear_acceleration));
    ...
    void handle_image(sensor_msgs::msg::Image::ConstSharedPtr msg) {
        ...
        try {
            ...
            payload.put("data",
                        base64_encode(msg->data.data(), msg->data.size()));
            RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 5000,
                                 "Falling back to raw image payload: %s",
                                 e.what());
        }
    ...
    std::vector<uint8_t>
    encode_to_jpeg(const sensor_msgs::msg::Image::ConstSharedPtr &msg) {
    ...
    void poll_command_bus() {
        while (rclcpp::ok()) {
            ...
            const std::string payload(static_cast<const char *>(message.data()),
                                      message.size());
            if (one_way_delay_.count() <= 0) {
            ...
            const auto ready_at =
                std::chrono::steady_clock::now() + one_way_delay_;
    ...
    void flush_delayed_commands() {
        ...
        while (!queued_commands_.empty() &&
               queued_commands_.front().ready_at <= now) {
    ...
    void process_command(const std::string &payload) {
        try {
            ...
            RCLCPP_WARN(get_logger(), "Failed to parse teleop command JSON: %s",
                        e.what());
        } catch (const std::exception &e) {
            RCLCPP_WARN(get_logger(), "Failed to process teleop command: %s",
                        e.what());
        }
    }
    ...
    void handle_cmd_vel_command(const pt::ptree &root, const pt::ptree &data) {
        try {
            ...
            // Determine input frame: optional override from JSON, else default
            // param.
            ...
            if (allow_cmd_frame_override_) {
                ...
                const auto root_frame =
                    root.get_optional<std::string>("frame_id");
                const auto data_frame =
                    data.get_optional<std::string>("frame_id");
            ...
            if (last_cmd_time_) {
                const auto dt =
                    std::chrono::duration<double>(now - *last_cmd_time_)
                        .count();
                if (twist.linear.x > 0.0) {
                    RCLCPP_INFO(get_logger(),
                                "Received /space_cobot/cmd_vel (dt=%.3fs)", dt);
                }
            }
            last_cmd_time_ = now;
            ...
            RCLCPP_WARN(get_logger(),
                        "Failed to process /space_cobot/cmd_vel command: %s",
                        e.what());
        }
    }
    ...
    void handle_nav6d_goal_command(const pt::ptree &data) {
        try {
            ...
            if (const auto header = data.get_child_optional("header")) {
                goal_msg.header.frame_id = header->get<std::string>(
            ...
            RCLCPP_WARN(get_logger(),
                        "Failed to process /nav6d/goal command: %s", e.what());
        }
    }
    ...
    void handle_nav6d_path_command(const pt::ptree &data) {
        try {
            ...
            if (path_msg.header.stamp.sec == 0 &&
            ...
            RCLCPP_WARN(get_logger(),
                        "Failed to process /nav6d/planner/path command: %s",
                        e.what());
        }
    }
    ...
    rclcpp::Subscription<nav6d::msg::PathExecutionSummary>::SharedPtr
        velocity_summary_sub_;
    rclcpp::Subscription<nav6d::msg::PathExecutionSummary>::SharedPtr
        force_summary_sub_;
